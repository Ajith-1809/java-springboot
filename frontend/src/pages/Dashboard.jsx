...
export default function Dashboard() {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (editingEmp) {
        await updateEmployee(editingEmp.id, finalData);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await deleteEmployee(showDeleteModal.id);
                      ...
                      {isAdmin && (
                        <td>
                          <div style={{ display: 'flex', gap: '0.4rem' }}>
                            <button className="btn" style={{ padding: '0.3rem' }} onClick={() => openModal(emp)}><Edit2 size={12} /></button>
                            <button className="btn" style={{ padding: '0.3rem' }} onClick={() => setShowDeleteModal(emp)}><Trash2 size={12} /></button>
                          </div>
                        </td>
                      )}
                    </tr>
